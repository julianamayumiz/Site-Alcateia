...
// ===================== COMUNICADOS =====================
var CAT_LABELS = { aviso: 'Aviso', evento: 'Evento', urgente: 'Urgente', informativo: 'Informativo' };

function renderComunicados() {
  var el = document.getElementById('com-lista-pais');
  if (!fbLoaded) {
    el.innerHTML = '<div class="empty"><div class="icon">⏳</div><p>Carregando comunicados...</p></div>';
    return;
  }
  ...
  if (!coms.length) {
    el.innerHTML = '<div class="empty"><div class="icon">📢</div><p>Nenhum comunicado por enquanto.<br>Volte em breve!</p></div>';
    return;
  }
...
function renderCalendario() {
  ...
  if (!cal.length) {
    el.innerHTML = '<div class="empty"><div class="icon">📅</div><p>Nenhuma atividade encontrada.</p></div>';
    return;
  }
...
function renderConfirmar() {
  ...
  if (!proximasAtiv.length) {
    el.innerHTML = '<div class="empty"><div class="icon">📅</div><p>Nenhuma atividade próxima para confirmar.</p></div>';
    return;
  }
  ...
  if (confirmado) {
    el.innerHTML = `<div class="success-box">
      ...
      <button onclick="novaConfirmacao()" style="margin-top:16px;padding:10px 20px;background:var(--accent);color:#fff;border:none;border-radius:var(--radius-sm);cursor:pointer;font-size:13px;font-family:'DM Sans',sans-serif">Confirmar outra atividade</button>
    </div>`;
    return;
  }
  ...
  el.innerHTML = `
    <div class="conf-section">
      ...
      <div class="form-group">
        ...
        <textarea id="conf-obs" placeholder="Alguma informação para a chefia..." style="min-height:70px;resize:vertical"></textarea>
      </div>
